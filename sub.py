...
W0 = '\033[0;37m'
R0 = '\033[0;31m'
import json,os,requests,sys,socket
try:
	os.system('clear')
	print '''%s
  _________    ___.        .___      
...
'''%(C0,W0,C0,W0,C0,W0,C0)
	for site in open(sys.argv[1]).read().splitlines():
		...
		for sub in open('2').read().split('\n'):
			try:
				if sub in result_sub:continue
				else:
					result_sub.append(sub)
					open('sub-'+sys.argv[1]+'.txt','a+').write('http://'+sub+'\n')
					ip=socket.gethostbyname(sub)
					if ip in result_ip:continue
					else:result_ip.append(ip);open('ip-'+sys.argv[1]+'.txt','a+').write(ip+'\n')
			except socket.gaierror:
				pass
		os.system('rm -rf 2')
		print '%s[%s✓%s] %s subdomains found for %s'%(W0,G0,W0,len(open('sub-'+sys.argv[1]+'.txt').read().split('\n')),site)
		print '%s[%s✓%s] %s ip found for %s\n'%(W0,G0,W0,len(open('ip-'+sys.argv[1]+'.txt').read().split('\n')),site)
	print '%s[%s✓%s] Done, saved in sub-%s.txt ip-%s.txt'%(W0,G0,W0,sys.argv[1],sys.argv[1])
